...
-- Tawiri with provider but capitalized, such as 'Stalmans'
-- Aerial Census in the Selous - Niassa Wildlife Corridor with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
--  with dataset id (table name) e.g 'gorongosa_flora'
-- 2010 with year of publication e.g. '2010' 
-- http://www.selous-niassa-corridor.org/uploads/tx_drblob/storage/Selous_Niassa_WC_2009_Census_Report_final.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- tawiri-cimu@habari.co.tz with the email address of the author or organization
...
delete from providers where provider = 'tawiri'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'tawiri', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	null, --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	null, --table_name: the name of the table that holds the species list. note same as dataset_id
	'snwc_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'tawiri', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Aerial Census in the Selous - Niassa Wildlife Corridor', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = '' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	null, --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = '' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	null, --data_table - the name of the species list table. should match data_registry.table_name
	'''Aerial Census in the Selous - Niassa Wildlife Corridor''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2, --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = '' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	null, --data_table - the name of the species list table. should match data_registry.table_name
	'''Tawiri, 2010''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = '' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	null, --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.selous-niassa-corridor.org/uploads/tx_drblob/storage/Selous_Niassa_WC_2009_Census_Report_final.pdf''''>www.selous-niassa-corridor.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = ''
...
values (
	...
	'http://www.selous-niassa-corridor.org/uploads/tx_drblob/storage/Selous_Niassa_WC_2009_Census_Report_final.pdf', --url - url of the datasource
