...
--Replace All 
-- mikumi_mammals with the species table name
...
alter table mikumi_mammals
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists mikumi_mammals_geom_id_btree;
create index mikumi_mammals_geom_id_btree ON mikumi_mammals (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists mikumi_mammals_scientificname_btree;
create index mikumi_mammals_scientificname_btree ON mikumi_mammals (scientificname);
...
SELECT  m.dist
FROM    mikumi_mammals m 
LEFT JOIN mikumi_mammals_geom g
...
update mikumi_mammals m
set geom_id = g.cartodb_id
from mikumi_mammals_geom g
...
SELECT * FROM get_tile('stanley', 'localinv', 'Crocidura elgonius','mikumi_mammals')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'stanley' and type = 'localinv' or table_name = 'mikumi_mammals'

SELECT d.*,g.*
  FROM mikumi_mammals d
  JOIN mikumi_mammals_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from mikumi_mammals where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('mikumi_mammals');
...
select distinct scientificname from mikumi_mammals

select distinct m.scientificname as n,
	t.common_names_eng as v
from mikumi_mammals m left join taxonomy t 
