...
--Replace All 
-- lormagolagrebo_plants with the species table name
...
alter table lormagolagrebo_plants
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists lormagolagrebo_plants_geom_id_btree;
create index lormagolagrebo_plants_geom_id_btree ON lormagolagrebo_plants (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lormagolagrebo_plants_scientificname_btree;
create index lormagolagrebo_plants_scientificname_btree ON lormagolagrebo_plants (scientificname);
...
from lormagolagrebo_plants_shapefile
group by geom_name;
...
update lormagolagrebo_plants m
...
SELECT * FROM get_tile('hoke', 'localinv', 'Antiaris toxicaria','lormagolagrebo_plants')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'hoke' and type = 'localinv' or table_name = 'lormagolagrebo_plants'

SELECT d.*,g.*
  FROM lormagolagrebo_plants d
  JOIN lormagolagrebo_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lormagolagrebo_plants where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lormagolagrebo_plants');
...
select distinct scientificname from lormagolagrebo_plants

select distinct m.scientificname as n,
	t.common_names_eng as v
from lormagolagrebo_plants m left join taxonomy t 
...
--species to test:
--Antiaris toxicaria (1 geom)
--Piper guineense (3 geom)
