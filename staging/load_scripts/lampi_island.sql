...
--Replace All 
-- lampi_island with the species table name
...
alter table lampi_island
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists lampi_island_geom_id_btree;
create index lampi_island_geom_id_btree ON lampi_island (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lampi_island_scientificname_btree;
create index lampi_island_scientificname_btree ON lampi_island (scientificname);
...
from wdpa2010
where new_new_name = 'Lampi Marine National Park'
...
update lampi_island m
set geom_id = g.cartodb_id
from lampi_island_geom g
...
SELECT * FROM get_tile('Beffasti', 'localinv', 'Ceratium dens','lampi_island')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'Beffasti' and type = 'localinv' or table_name = 'lampi_island'

SELECT d.*,g.*
  FROM lampi_island d
  JOIN lampi_island_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lampi_island where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lampi_island');
...
select distinct scientificname from lampi_island

select distinct m.scientificname as n,
	t.common_names_eng as v
from lampi_island m left join taxonomy t 
