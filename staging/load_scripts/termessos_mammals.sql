...
--Replace All 
-- termessos_mammals with the species table name
...
alter table termessos_mammals
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists termessos_mammals_geom_id_btree;
create index termessos_mammals_geom_id_btree ON termessos_mammals (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists termessos_mammals_scientificname_btree;
create index termessos_mammals_scientificname_btree ON termessos_mammals (scientificname);
...
from termessos_mammals_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    termessos_mammals m 
LEFT JOIN termessos_mammals_geom g
...
update termessos_mammals m
set geom_id = g.cartodb_id
from termessos_mammals_geom g
...
SELECT * FROM get_tile('marinis', 'localinv', 'Erinaceus concolor','termessos_mammals')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'marinis' and type = 'localinv' or table_name = 'termessos_mammals'

SELECT d.*,g.*
  FROM termessos_mammals d
  JOIN termessos_mammals_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from termessos_mammals where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('termessos_mammals');
...
select distinct scientificname from termessos_mammals

select distinct m.scientificname as n,
	t.common_names_eng as v
from termessos_mammals m left join taxonomy t 
