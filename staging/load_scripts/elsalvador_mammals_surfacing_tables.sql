...
-- Owen with provider but capitalized, such as 'Stalmans'
-- Revised Checklist and Distributions of Land Mammals of El Salvador with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- elsalvador_mammals with dataset id (table name) e.g 'gorongosa_flora'
-- 2012 with year of publication e.g. '2010' 
-- http://www.nsrl.ttu.edu/publications/opapers/ops/OP310.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.nsrl.ttu.edu with the short url to the study e.g. oo.adu.org.za
-- mammals with the lower-case tax. e.g. herps or aves
-- Mammals with the same as taxa column, but capitalize
-- owenesmeralda@gmail.com with the email address of the author or organization
-- El Salvadaor - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 1991 to 2010 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- GADM - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Wilson and Reeder (2005) with references for the taxonomy

delete from providers where provider = 'owen'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'owen', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'elsalvador_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'elsalvador_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'elsalvador_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'owen', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Revised Checklist and Distributions of Land Mammals of El Salvador', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'elsalvador_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'elsalvador_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'elsalvador_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'elsalvador_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Revised Checklist and Distributions of Land Mammals of El Salvador''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'elsalvador_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'elsalvador_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Owen, 2012''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'elsalvador_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'elsalvador_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.nsrl.ttu.edu/publications/opapers/ops/OP310.pdf''''>www.nsrl.ttu.edu</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'elsalvador_mammals'
...
values (
	...
	'owen', --provider
	'owenesmeralda@gmail.com', --contact - email address of contact
	'El Salvadaor', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'elsalvador_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 1991 to 2010', --ca. 1991 to 2010 with a string representation for the date range "ca. 1980 to 2010"
	'Revised Checklist and Distributions of Land Mammals of El Salvador', --description - string description
	...
	'Wilson and Reeder (2005)', --taxonomy_metadata: references for the taxonomy
	'http://www.nsrl.ttu.edu/publications/opapers/ops/OP310.pdf', --url - url of the datasource
