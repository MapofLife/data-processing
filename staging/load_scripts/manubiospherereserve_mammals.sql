...
--Replace All 
-- manubiospherereserve_mammals with the species table name
...
alter table manubiospherereserve_mammals
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists manubiospherereserve_mammals_geom_id_btree;
create index manubiospherereserve_mammals_geom_id_btree ON manubiospherereserve_mammals (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists manubiospherereserve_mammals_scientificname_btree;
create index manubiospherereserve_mammals_scientificname_btree ON manubiospherereserve_mammals (scientificname);
...
from manubiospherereserve_mammals_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    manubiospherereserve_mammals m 
LEFT JOIN manubiospherereserve_mammals_geom g
...
update manubiospherereserve_mammals m
set geom_id = g.cartodb_id
from manubiospherereserve_mammals_geom g
...
SELECT * FROM get_tile('pacheco', 'localinv', 'Marmosa rubra','manubiospherereserve_mammals')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'pacheco' and type = 'localinv' or table_name = 'manubiospherereserve_mammals'

SELECT d.*,g.*
  FROM manubiospherereserve_mammals d
  JOIN manubiospherereserve_mammals_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from manubiospherereserve_mammals where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('manubiospherereserve_mammals');
...
select distinct scientificname from manubiospherereserve_mammals

select distinct m.scientificname as n,
	t.common_names_eng as v
from manubiospherereserve_mammals m left join taxonomy t 
