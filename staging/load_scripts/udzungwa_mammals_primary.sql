...
--Replace All 
-- udzungwa_mammals_primary with the species table name
...
alter table udzungwa_mammals_primary
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists udzungwa_mammals_primary_geom_id_btree;
create index udzungwa_mammals_primary_geom_id_btree ON udzungwa_mammals_primary (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists udzungwa_mammals_primary_scientificname_btree;
create index udzungwa_mammals_primary_scientificname_btree ON udzungwa_mammals_primary (scientificname);
...
from wdpa2010
where new_new_name = 'Udzungwa Mountains'
...
from udzungwa_mammals_primary_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    udzungwa_mammals_primary m 
LEFT JOIN udzungwa_mammals_primary_geom g
...
update udzungwa_mammals_primary m
set geom_id = g.cartodb_id
from udzungwa_mammals_primary_geom g
...
from udzungwa_mammals_primary_geom group by valid_geom, valid_geom_wm
...
SELECT * FROM get_tile('rovero', 'localinv', 'Cercocebus galeritus sanjei','udzungwa_mammals_primary')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'rovero' and type = 'localinv' or table_name = 'udzungwa_mammals_primary'

SELECT d.*,g.*
  FROM udzungwa_mammals_primary d
  JOIN udzungwa_mammals_primary_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from udzungwa_mammals_primary where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('udzungwa_mammals_primary');
...
select distinct scientificname from udzungwa_mammals_primary

select distinct m.scientificname as n,
	t.common_names_eng as v
from udzungwa_mammals_primary m left join taxonomy t 
