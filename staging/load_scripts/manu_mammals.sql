...
--Replace All 
-- manu_mammals with the species table name
...
alter table manu_mammals
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists manu_mammals_geom_id_btree;
create index manu_mammals_geom_id_btree ON manu_mammals (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists manu_mammals_scientificname_btree;
create index manu_mammals_scientificname_btree ON manu_mammals (scientificname);
...
from wdpa2010
where new_new_name = 'Manu' and country = 'PER'
...
update manu_mammals m
set geom_id = g.cartodb_id
from manu_mammals_geom g
...
SELECT * FROM get_tile('terborgh', 'localinv', 'Tayassu tajacu','manu_mammals')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'terborgh' and type = 'localinv' or table_name = 'manu_mammals'

SELECT d.*,g.*
  FROM manu_mammals d
  JOIN manu_mammals_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from manu_mammals where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('manu_mammals');
...
select distinct scientificname from manu_mammals

select distinct m.scientificname as n,
	t.common_names_eng as v
from manu_mammals m left join taxonomy t 
