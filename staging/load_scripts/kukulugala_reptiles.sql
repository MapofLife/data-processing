...
--Replace All 
-- kukulugala_reptiles with the species table name
...
alter table kukulugala_reptiles
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists kukulugala_reptiles_geom_id_btree;
create index kukulugala_reptiles_geom_id_btree ON kukulugala_reptiles (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists kukulugala_reptiles_scientificname_btree;
create index kukulugala_reptiles_scientificname_btree ON kukulugala_reptiles (scientificname);
...
FROM wdpa2010 where wdpaid = 27915
...
from kukulugala_reptiles_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    kukulugala_reptiles m 
LEFT JOIN kukulugala_reptiles_geom g
...
update kukulugala_reptiles m
set geom_id = g.cartodb_id
from kukulugala_reptiles_geom g
...
SELECT * FROM get_tile('karunarathna', 'localinv', 'Calotes calotes','kukulugala_reptiles')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'karunarathna' and type = 'localinv' or table_name = 'kukulugala_reptiles'

SELECT d.*,g.*
  FROM kukulugala_reptiles d
  JOIN kukulugala_reptiles_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from kukulugala_reptiles where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('kukulugala_reptiles');
...
select distinct scientificname from kukulugala_reptiles

select distinct m.scientificname as n,
	t.common_names_eng as v
from kukulugala_reptiles m left join taxonomy t 
