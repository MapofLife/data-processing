...
--Replace All 
-- xebang_birdsmammals with the species table name
...
alter table xebang_birdsmammals
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists xebang_birdsmammals_geom_id_btree;
create index xebang_birdsmammals_geom_id_btree ON xebang_birdsmammals (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists xebang_birdsmammals_scientificname_btree;
create index xebang_birdsmammals_scientificname_btree ON xebang_birdsmammals (scientificname);
...
from wdpa2010
where new_new_name = 'Xe Bang Nouan'
...
from xebang_birdsmammals_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    xebang_birdsmammals m 
LEFT JOIN xebang_birdsmammals_geom g
...
update xebang_birdsmammals m
set geom_id = g.cartodb_id
from xebang_birdsmammals_geom g
...
SELECT * FROM get_tile('timmins', 'localinv', 'Spilornis cheela','xebang_birdsmammals')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'timmins' and type = 'localinv' or table_name = 'xebang_birdsmammals'

SELECT d.*,g.*
  FROM xebang_birdsmammals d
  JOIN xebang_birdsmammals_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from xebang_birdsmammals where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('xebang_birdsmammals');
...
select distinct scientificname from xebang_birdsmammals

select distinct m.scientificname as n,
	t.common_names_eng as v
from xebang_birdsmammals m left join taxonomy t 
