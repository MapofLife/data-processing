...
-- Matveev with provider but capitalized, such as 'Stalmans'
-- Checklist of Cambodian bats (Chiroptera), with new records and remakrs on taxonomy  with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- cambodia_mammals_primary with dataset id (table name) e.g 'gorongosa_flora'
-- 2005 with year of publication e.g. '2010' 
-- http://zmmu.msu.ru/rjt/articles/ther4_1%2043_62%20Matveev.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- zmmu.msu.ru with the short url to the study e.g. oo.adu.org.za
-- mammals with the lower-case tax. e.g. herps or aves
-- Mammals with the same as taxa column, but capitalize
-- vital-m@mail.ru with the email address of the author or organization
-- Cambodia - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 200 to 2002 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- GADM - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Mammals: Metveev (1999), Kock (2000), Hendrichsen et al. (2001) with references for the taxonomy
...
select
	'matveev', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
where not exists (select * from providers where provider = 'matveev')
...
values (
	'cambodia_mammals_compiled', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'cambodia_mammals_compiled', --table_name: the name of the table that holds the species list. note same as dataset_id
	'cambodia_mammals_compiled_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'matveev', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Checklist of Cambodian bats (Chiroptera), with new records and remarks on taxonomy ', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'cambodia_mammals_primary' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'cambodia_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'cambodia_mammals_primary' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'cambodia_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''Checklist of Cambodian bats (Chiroptera), with new records and remarks on taxonomy ''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'cambodia_mammals_primary' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'cambodia_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''Matveev, 2005''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'cambodia_mammals_primary' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'cambodia_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://zmmu.msu.ru/rjt/articles/ther4_1%2043_62%20Matveev.pdf''''>zmmu.msu.ru</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'cambodia_mammals_primary'
...
values (
	...
	'matveev', --provider
	'vital-m@mail.ru', --contact - email address of contact
	'Cambodia', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'cambodia_mammals_compiled', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2000 to 2002', --ca. 200 to 2002 with a string representation for the date range "ca. 1980 to 2010"
	'Checklist of Cambodian bats (Chiroptera), with new records and remarks on taxonomy ', --description - string description
	...
	'Mammals: Metveev (1999), Kock (2000), Hendrichsen et al. (2001)', --taxonomy_metadata: references for the taxonomy
	'http://zmmu.msu.ru/rjt/articles/ther4_1%2043_62%20Matveev.pdf', --url - url of the datasource
