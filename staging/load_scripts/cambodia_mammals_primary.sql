...
--Replace All 
-- cambodia_mammals_primary with the species table name
...
alter table cambodia_mammals_primary
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists cambodia_mammals_primary_geom_id_btree;
create index cambodia_mammals_primary_geom_id_btree ON cambodia_mammals_primary (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists cambodia_mammals_primary_scientificname_btree;
create index cambodia_mammals_primary_scientificname_btree ON cambodia_mammals_primary (scientificname);
...
SELECT  m.dist
FROM    cambodia_mammals_primary m 
LEFT JOIN cambodia_mammals_primary_geom g
...
update cambodia_mammals_primary m
set geom_id = g.cartodb_id
from cambodia_mammals_primary_geom g
...
from cambodia_mammals_primary_geom group by valid_geom, valid_geom_wm
...
SELECT * FROM get_tile('matveev', 'localinv', 'Pteropus hypomelanus','cambodia_mammals_primary')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'matveev' and type = 'localinv' or table_name = 'cambodia_mammals_primary'

SELECT d.*,g.*
  FROM cambodia_mammals_primary d
  JOIN cambodia_mammals_primary_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from cambodia_mammals_primary where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('cambodia_mammals_primary');
...
select distinct scientificname from cambodia_mammals_primary

select distinct m.scientificname as n,
	t.common_names_eng as v
from cambodia_mammals_primary m left join taxonomy t 
