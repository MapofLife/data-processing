...
--Replace All 
-- lormagolagrebo_birds with the species table name
...
alter table lormagolagrebo_birds
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists lormagolagrebo_birds_geom_id_btree;
create index lormagolagrebo_birds_geom_id_btree ON lormagolagrebo_birds (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lormagolagrebo_birds_scientificname_btree;
create index lormagolagrebo_birds_scientificname_btree ON lormagolagrebo_birds (scientificname);
...
from wdpa2010
where new_new_name = 'North Lorma'
union
select new_new_name as geom_name, the_geom
from wdpa2010
where new_new_name = 'Gola'
union
select new_new_name as geom_name, the_geom
from wdpa2010
where new_new_name = 'Grebo'
...
from lormagolagrebo_birds_shapefile
group by geom_name;
...
SELECT * FROM get_tile('hoke', 'localinv', 'Ciconia episcopus','lormagolagrebo_birds')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'hoke' and type = 'localinv' or table_name = 'lormagolagrebo_birds'

SELECT d.*,g.*
  FROM lormagolagrebo_birds d
  JOIN lormagolagrebo_birds_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lormagolagrebo_birds where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lormagolagrebo_birds');
...
select distinct scientificname from lormagolagrebo_birds

select distinct m.scientificname as n,
	t.common_names_eng as v
from lormagolagrebo_birds m left join taxonomy t 
...
--species to test:
--Ciconia episcopus (1 geometry)
--Gypohierax angolensis (all 3 geometries)
