...
--Replace All 
-- lorelindusulawesi_herps_compiled with the species table name
...
alter table lorelindusulawesi_herps_compiled
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists lorelindusulawesi_herps_compiled_geom_id_btree;
create index lorelindusulawesi_herps_compiled_geom_id_btree ON lorelindusulawesi_herps_compiled (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lorelindusulawesi_herps_compiled_scientificname_btree;
create index lorelindusulawesi_herps_compiled_scientificname_btree ON lorelindusulawesi_herps_compiled (scientificname);
...
from lorelindusulawesi_herps_compiled_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    lorelindusulawesi_herps_compiled m 
LEFT JOIN lorelindusulawesi_herps_compiled_geom g
...
update lorelindusulawesi_herps_compiled m
set geom_id = g.cartodb_id
from lorelindusulawesi_herps_compiled_geom g
...
from lorelindusulawesi_herps_compiled_geom group by valid_geom, valid_geom_wm
...
SELECT * FROM get_tile('wanger', 'localinv', 'Bronchocela celebensis','lorelindusulawesi_herps_compiled')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'wanger' and type = 'localinv' or table_name = 'lorelindusulawesi_herps_compiled'

SELECT d.*,g.*
  FROM lorelindusulawesi_herps_compiled d
  JOIN lorelindusulawesi_herps_compiled_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lorelindusulawesi_herps_compiled where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lorelindusulawesi_herps_compiled');
...
select distinct scientificname from lorelindusulawesi_herps_compiled

select distinct m.scientificname as n,
	t.common_names_eng as v
from lorelindusulawesi_herps_compiled m left join taxonomy t 
