...
-- Onn with provider but capitalized, such as 'Stalmans'
-- First Report on the Herpetofauna of Pulau Pangkor, Perak, Malaysia  with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- pulaupangkor_herps with dataset id (table name) e.g 'gorongosa_flora'
-- 2010 with year of publication e.g. '2010' 
-- http://www.academia.edu/938499/First_report_on_herpetofauna_of_Pulau_Pangkor_Perak_Malaysia with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- kin_onn@yahoo.com with the email address of the author or organization
-- Pulau Pangkor - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2009 to 2010 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- GADM - what was used for spatial information
-- Amphibians - same as class but with common names "Plants", or "Birds"
-- Hemidactylus:Carranza and Arnold (2006);Draco: Manthey (2008); Amphibian: Frost (2009): with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'onn'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'onn', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'pulaupangkor_herps', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'pulaupangkor_herps', --table_name: the name of the table that holds the species list. note same as dataset_id
	'pulaupangkor_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'onn', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'First Report on the Herpetofauna of Pulau Pangkor, Perak, Malaysia ', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'pulaupangkor_herps' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'pulaupangkor_herps', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'pulaupangkor_herps' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'pulaupangkor_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''First Report on the Herpetofauna of Pulau Pangkor, Perak, Malaysia ''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'pulaupangkor_herps' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'pulaupangkor_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Onn, 2010''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'pulaupangkor_herps' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'pulaupangkor_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.academia.edu/938499/First_report_on_herpetofauna_of_Pulau_Pangkor_Perak_Malaysia''''>www.academia.edu</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'pulaupangkor_herps'
...
values (
	...
	'onn', --provider
	'kin_onn@yahoo.com', --contact - email address of contact
	'Pulau Pangkor', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'pulaupangkor_herps', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2009 to 2010', --ca. 2009 to 2010 with a string representation for the date range "ca. 1980 to 2010"
	'First Report on the Herpetofauna of Pulau Pangkor, Perak, Malaysia ', --description - string description
	null, --recommended_citation 
	'none', -- none - null if none
	null, --  - null if none
	'GADM', -- GADM - what was used for spatial information
	'Amphibians', -- Amphibians - same as class but with common names "Plants", or "Birds"
	'Hemidactylus:Carranza and Arnold (2006);Draco: Manthey (2008); Amphibian: Frost (2009):', --taxonomy_metadata: references for the taxonomy
	'http://www.academia.edu/938499/First_report_on_herpetofauna_of_Pulau_Pangkor_Perak_Malaysia', --url - url of the datasource
