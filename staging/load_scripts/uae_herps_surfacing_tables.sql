...
-- Soorae with provider but capitalized, such as 'Stalmans'
-- An Overview and Checklist of the Native and Alien Herpetofauna of the United Arab Emirates with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- uae_herps with dataset id (table name) e.g 'gorongosa_flora'
-- 2010 with year of publication e.g. '2010' 
-- http://www.podarcis.de/AS/Bibliografie/BIB_5324.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.podarcis.de with the short url to the study e.g. oo.adu.org.za
-- herps with the lower-case tax. e.g. herps or aves
-- Herps with the same as taxa column, but capitalize
-- psoorae@ead.ae with the email address of the author or organization
-- United Arab Emirated - the geographic extent eg. "Global" or "Angola" or "New World"
...
-- Amphibians: Arnold (1984, 1986), Balletto et al. (1985), Gasperetti (1988), Leviton et al. (1992), Gasperetti et al. (1993), Egan (2007), Gardner (2005, 2008) with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'soorae'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'soorae', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'uae_herps', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'uae_herps', --table_name: the name of the table that holds the species list. note same as dataset_id
	'uae_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'soorae', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'An Overview and Checklist of the Native and Alien Herpetofauna of the United Arab Emirates', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'uae_herps' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'uae_herps', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'uae_herps' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'uae_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''An Overview and Checklist of the Native and Alien Herpetofauna of the United Arab Emirates''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'uae_herps' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'uae_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Soorae, 2010''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'uae_herps' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'uae_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.podarcis.de/AS/Bibliografie/BIB_5324.pdf''''>www.podarcis.de</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'uae_herps'
...
values (
	...
	'An Overview and Checklist of the Native and Alien Herpetofauna of the United Arab Emirates', --description - string description
	...
	'Amphibians: Arnold (1984, 1986), Balletto et al. (1985), Gasperetti (1988), Leviton et al. (1992), Gasperetti et al. (1993), Egan (2007), Gardner (2005, 2008)', --taxonomy_metadata: references for the taxonomy
	'http://www.podarcis.de/AS/Bibliografie/BIB_5324.pdf', --url - url of the datasource
