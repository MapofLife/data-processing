...
-- Results of a preliminary small mammal survey of Malundwe Mountain, Mikumi National Park, Tanzania with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- mikumi_mammals with dataset id (table name) e.g 'gorongosa_flora'
-- 2005 with year of publication e.g. '2010' 
-- http://www.bioone.org/doi/abs/10.2982/0012-8317%282005%2994%5B213:ROAPSM%5D2.0.CO;2 with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- Mikumi National Park - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2004 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- WDPA 2010 - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Mammals: Hutterer (1993), Koopman (1993), Musser & Carleton (1993) with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'stanley'
...
values (
	'mikumi_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'mikumi_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'mikumi_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	...
	'Results of a preliminary small mammal survey of Malundwe Mountain, Mikumi National Park, Tanzania', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'mikumi_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'mikumi_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'mikumi_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'mikumi_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Results of a preliminary small mammal survey of Malundwe Mountain, Mikumi National Park, Tanzania''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'mikumi_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- delete from feature_metadata where data_table = 'mikumi_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'mikumi_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.bioone.org/doi/abs/10.2982/0012-8317%282005%2994%5B213:ROAPSM%5D2.0.CO;2''''>www.bioone.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'mikumi_mammals'
...
values (
	...
	'Mikumi National Park', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'mikumi_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2004', --ca. 2004 with a string representation for the date range "ca. 1980 to 2010"
	'Results of a preliminary small mammal survey of Malundwe Mountain, Mikumi National Park, Tanzania', --description - string description
	...
	'Mammals: Hutterer (1993), Koopman (1993), Musser & Carleton (1993)', --taxonomy_metadata: references for the taxonomy
	'http://www.bioone.org/doi/abs/10.2982/0012-8317%282005%2994%5B213:ROAPSM%5D2.0.CO;2', --url - url of the datasource
