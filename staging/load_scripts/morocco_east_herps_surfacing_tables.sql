...
-- Barata with provider but capitalized, such as 'Stalmans'
-- New observations of amphibians and reptiles in Morocco, with a special emphasis on the eastern region with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- morocco_east_herps with dataset id (table name) e.g 'gorongosa_flora'
-- 2011 with year of publication e.g. '2010' 
-- http://www.arievandermeijden.nl/wordpress/wp-content/uploads/2011/08/Barata2011_Herpetological-Bulletin_final.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.arievandermeijden.nl with the short url to the study e.g. oo.adu.org.za
-- herps with the lower-case tax. e.g. herps or aves
-- Herps with the same as taxa column, but capitalize
-- mrbarata@gmail.com with the email address of the author or organization
-- Morocco - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2008 to 2009 with a string representation for the date range "ca. 1980 to 2010"
-- 
--  - null if none
--  - null if none
...
delete from providers where provider = 'barata'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'barata', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'morocco_east_herps', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'morocco_east_herps', --table_name: the name of the table that holds the species list. note same as dataset_id
	'morocco_east_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'barata', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'New observations of amphibians and reptiles in Morocco, with a special emphasis on the eastern region', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'morocco_east_herps' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,provider,field,title,"order","type") --order and type are keywords so need to be in quotes
values (
	'morocco_east_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'barata', --provider - the id of the provider.  should match providers.provider
...
values (
	'morocco_east_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'barata', --provider - the id of the provider.  should match providers.provider
	'''New observations of amphibians and reptiles in Morocco, with a special emphasis on the eastern region''', --field - the name of the field to be displayed from the data_table
...
values (
	'morocco_east_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'barata', --provider - the id of the provider.  should match providers.provider
	'''Barata, 2011''', --field - the name of the field to be displayed from the data_table
...
values (
	'morocco_east_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'barata', --provider - the id of the provider.  should match providers.provider
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.arievandermeijden.nl/wordpress/wp-content/uploads/2011/08/Barata2011_Herpetological-Bulletin_final.pdf''''>www.arievandermeijden.nl</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'morocco_east_herps'
...
values (
	...
	'barata', --provider
	'mrbarata@gmail.com', --contact - email address of contact
	'Morocco', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'morocco_east_herps', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2008 to 2009', --ca. 2008 to 2009 with a string representation for the date range "ca. 1980 to 2010"
	'New observations of amphibians and reptiles in Morocco, with a special emphasis on the eastern region', --description - string description
