...
--Replace All 
-- udzungwa_mammals_compiled with the species table name
...
alter table udzungwa_mammals_compiled
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists udzungwa_mammals_compiled_geom_id_btree;
create index udzungwa_mammals_compiled_geom_id_btree ON udzungwa_mammals_compiled (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists udzungwa_mammals_compiled_scientificname_btree;
create index udzungwa_mammals_compiled_scientificname_btree ON udzungwa_mammals_compiled (scientificname);
...
from udzungwa_mammals_compiled_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    udzungwa_mammals_compiled m 
LEFT JOIN udzungwa_mammals_compiled_geom g
...
update udzungwa_mammals_compiled m
set geom_id = g.cartodb_id
from udzungwa_mammals_compiled_geom g
...
from udzungwa_mammals_compiled_geom group by valid_geom, valid_geom_wm
...
SELECT * FROM get_tile('rovero', 'localinv', 'Chrysochloris stuhlmanni','udzungwa_mammals_compiled')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'rovero' and type = 'localinv' or table_name = 'udzungwa_mammals_compiled'

SELECT d.*,g.*
  FROM udzungwa_mammals_compiled d
  JOIN udzungwa_mammals_compiled_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from udzungwa_mammals_compiled where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('udzungwa_mammals_compiled');
...
select distinct scientificname from udzungwa_mammals_compiled

select distinct m.scientificname as n,
	t.common_names_eng as v
from udzungwa_mammals_compiled m left join taxonomy t 
