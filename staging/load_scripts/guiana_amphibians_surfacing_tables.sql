...
-- Senaris with provider but capitalized, such as 'Stalmans'
...
-- guiana_amphibians with dataset id (table name) e.g 'gorongosa_flora'
...
-- amphibians with the lower-case tax. e.g. herps or aves
-- Amphibians with the same as taxa column, but capitalize
...
-- ca. 2001 to 2005 with a string representation for the date range "ca. 1980 to 2010"
-- Hollowell, T., and R. P. Reynolds, eds. 2005. Checklist of the Terrestrial Vertebrates of the Guiana Shield. Bulletin of the Biological Society of Washington, no. 13.
-- none - null if none
--  - null if none
-- GADM - what was used for spatial information
-- Amphibians - same as class but with common names "Plants", or "Birds"
-- Amphibians: Frost (2004) with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'Senaris'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'Senaris', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'guiana_amphibians', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'guiana_amphibians', --table_name: the name of the table that holds the species list. note same as dataset_id
	'guiana_amphibians_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'Senaris', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	'amphibians', --taxa: the taxonomic group, like "plants", "aves", herps, etc.  should match dashboard_metadata.class	
	'Amphibians', -- classes: same as taxa column, but capitalize
...
--delete from feature_metadata where data_table = 'guiana_amphibians' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'guiana_amphibians', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'guiana_amphibians' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- delete from feature_metadata where data_table = 'guiana_amphibians' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'guiana_amphibians', --data_table - the name of the species list table. should match data_registry.table_name
	'''Senaris, 2005''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'guiana_amphibians' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'guiana_amphibians'
...
values (
	'amphibians', --class - the taxonomic group, like "plants" or "aves".  should match data_registry.taxa
	'Senaris', --provider
	null, --contact - email address of contact
	'Guiana Shield', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'guiana_amphibians', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2001 to 2005', --ca. 2001 to 2005 with a string representation for the date range "ca. 1980 to 2010"
	'Checklist of the terrestrial vertebrates of the Guiana Shield', --description - string description
	'Hollowell, T., and R. P. Reynolds, eds. 2005. Checklist of the Terrestrial Vertebrates of the Guiana Shield. Bulletin of the Biological Society of Washington, no. 13.', --recommended_citation 
	...
	'Amphibians: Frost (2004)', --taxonomy_metadata: references for the taxonomy
