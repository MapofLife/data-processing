...
--Replace All 
-- lormagolagrebo_reptiles with the species table name
...
alter table lormagolagrebo_reptiles
...
create index lormagolagrebo_geom_id_btree ON lormagolagrebo_reptiles (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lormagolagrebo_reptiles_scientificname_btree;
create index lormagolagrebo_reptiles_scientificname_btree ON lormagolagrebo_reptiles (scientificname);
...
select the_geom from lormagolagrebo_geom
...
SELECT  m.dist
FROM    lormagolagrebo_reptiles m 
...
update lormagolagrebo_reptiles m
...
SELECT * FROM get_tile('hoke', 'localinv', 'Agama agama','lormagolagrebo_reptiles')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'hoke' and type = 'localinv' or table_name = 'lormagolagrebo_reptiles'

SELECT d.*,g.*
  FROM lormagolagrebo_reptiles d
  JOIN lormagolagrebo_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lormagolagrebo_reptiles where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lormagolagrebo_reptiles');
...
select distinct scientificname from lormagolagrebo_reptiles

select distinct m.scientificname as n,
	t.common_names_eng as v
from lormagolagrebo_reptiles m left join taxonomy t 
