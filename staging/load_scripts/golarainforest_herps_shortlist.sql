...
--Replace All 
-- golarainforest_herps_shortlist with the species table name
...
alter table golarainforest_herps_shortlist
...
create index golarainforest_herps_geom_id_btree ON golarainforest_herps_shortlist (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists golarainforest_herps_shortlist_scientificname_btree;
create index golarainforest_herps_shortlist_scientificname_btree ON golarainforest_herps_shortlist (scientificname);
...
SELECT  m.dist
FROM    golarainforest_herps_shortlist m 
LEFT JOIN golarainforest_herps_geom g
...
update golarainforest_herps_shortlist m
set geom_id = g.cartodb_id
from golarainforest_herps_geom g
...
SELECT * FROM get_tile('wardill', 'localinv', 'Geotrypetes seraphini','golarainforest_herps_shortlist')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'wardill' and type = 'localinv' or table_name = 'golarainforest_herps_shortlist'

SELECT d.*,g.*
  FROM golarainforest_herps_shortlist d
  JOIN golarainforest_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Leptopelis macrotis';

select * from golarainforest_herps_shortlist where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('golarainforest_herps_shortlist');
...
select distinct scientificname from golarainforest_herps_shortlist

select distinct m.scientificname as n,
	t.common_names_eng as v
from golarainforest_herps_shortlist m left join taxonomy t 
