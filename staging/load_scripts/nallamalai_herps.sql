...
--Replace All 
-- nallamalai_herps with the species table name
...
alter table nallamalai_herps
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists nallamalai_herps_geom_id_btree;
create index nallamalai_herps_geom_id_btree ON nallamalai_herps (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists nallamalai_herps_scientificname_btree;
create index nallamalai_herps_scientificname_btree ON nallamalai_herps (scientificname);
...
from nallamalai_herps_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    nallamalai_herps m 
LEFT JOIN nallamalai_herps_geom g
...
update nallamalai_herps m
set geom_id = g.cartodb_id
from nallamalai_herps_geom g
...
from nallamalai_herps_geom group by valid_geom, valid_geom_wm
...
SELECT * FROM get_tile('rao', 'localinv', 'Bufo melanostictus','nallamalai_herps')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'rao' and type = 'localinv' or table_name = 'nallamalai_herps'

SELECT d.*,g.*
  FROM nallamalai_herps d
  JOIN nallamalai_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from nallamalai_herps where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('nallamalai_herps');
...
select distinct scientificname from nallamalai_herps

select distinct m.scientificname as n,
	t.common_names_eng as v
from nallamalai_herps m left join taxonomy t 
