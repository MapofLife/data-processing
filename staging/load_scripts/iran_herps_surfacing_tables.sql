-- rastegar-pouyani with provider e.g. 'stalmans'
-- Rastegar-Pouyani with provider but capitalized, such as 'Stalmans'
-- Annotated Checklist of Amphibians and Reptiles of Iran with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- iran_herps with dataset id (table name) e.g 'gorongosa_flora'
-- 2008 with year of publication e.g. '2010' 
-- http://www.sid.ir/en/VEWSSID/J_pdf/116420080102.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.sid.ir with the short url to the study e.g. oo.adu.org.za
-- herps with the lower-case tax. e.g. herps or aves
-- Herps with the same as taxa column, but capitalize
-- nasrullah.r@gmail.com with the email address of the author or organization
-- Iran - the geographic extent eg. "Global" or "Angola" or "New World"
...
select * from providers where provider = 'rastegar-pouyani'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'rastegar-pouyani', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'iran_herps', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'iran_herps', --table_name: the name of the table that holds the species list. note same as dataset_id
	'iran_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'rastegar-pouyani', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Annotated Checklist of Amphibians and Reptiles of Iran', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'iran_herps' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'iran_herps', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'iran_herps' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'iran_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Annotated Checklist of Amphibians and Reptiles of Iran''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'iran_herps' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'iran_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Rastegar-Pouyani, 2008''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'iran_herps' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'iran_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.sid.ir/en/VEWSSID/J_pdf/116420080102.pdf''''>www.sid.ir</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'iran_herps'
...
values (
	...
	'rastegar-pouyani', --provider
	'nasrullah.r@gmail.com', --contact - email address of contact
	'Iran', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'iran_herps', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2008', --ca. 2008 with a string representation for the date range "ca. 1980 to 2010"
	'Annotated Checklist of Amphibians and Reptiles of Iran', --description - string description
	...
	'http://www.sid.ir/en/VEWSSID/J_pdf/116420080102.pdf', --url - url of the datasource
