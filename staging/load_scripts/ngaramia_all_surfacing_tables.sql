...
-- Doggart with provider but capitalized, such as 'Stalmans'
-- Ngaramia Riverine Forest: A Biodiversity Survey  with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- ngaramia_all with dataset id (table name) e.g 'gorongosa_flora'
-- 2002 with year of publication e.g. '2010' 
-- http://www.tfcg.org/pdf/TFCG-Ngaramia-biodiversity-survey-report.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.tfcg.org with the short url to the study e.g. oo.adu.org.za
-- all with the lower-case tax. e.g. herps or aves
-- All with the same as taxa column, but capitalize
--  with the email address of the author or organization
-- Ngaramia Forest - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2000 to 2001 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- created geometries using coordinates from paper - what was used for spatial information
-- All - same as class but with common names "Plants", or "Birds"
-- Mammals: Kingdon (1997); Birds: Van Perlo (1995);  with references for the taxonomy
...
select
	'doggart', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
where not exists (select * from providers where provider = 'doggart')
...
values (
	'ngaramia_all', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'ngaramia_all', --table_name: the name of the table that holds the species list. note same as dataset_id
	'ngaramia_all_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'doggart', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	'all', --taxa: the taxonomic group, like "plants", "aves", herps, etc.  should match dashboard_metadata.class	
	'All', -- classes: same as taxa column, but capitalize
	'Ngaramia Riverine Forest: A Biodiversity Survey ', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'ngaramia_all' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'ngaramia_all', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'ngaramia_all' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'ngaramia_all', --data_table - the name of the species list table. should match data_registry.table_name
	'''Ngaramia Riverine Forest: A Biodiversity Survey ''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'ngaramia_all' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'ngaramia_all', --data_table - the name of the species list table. should match data_registry.table_name
	'''Doggart, 2002''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'ngaramia_all' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'ngaramia_all', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.tfcg.org/pdf/TFCG-Ngaramia-biodiversity-survey-report.pdf''''>www.tfcg.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'ngaramia_all'
...
values (
	...
	'Ngaramia Forest', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'ngaramia_all', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2000 to 2001', --ca. 2000 to 2001 with a string representation for the date range "ca. 1980 to 2010"
	'Ngaramia Riverine Forest: A Biodiversity Survey ', --description - string description
	...
	'All', -- All - same as class but with common names "Plants", or "Birds"
	'Mammals: Kingdon (1997); Birds: Van Perlo (1995); ', --taxonomy_metadata: references for the taxonomy
	'http://www.tfcg.org/pdf/TFCG-Ngaramia-biodiversity-survey-report.pdf', --url - url of the datasource
