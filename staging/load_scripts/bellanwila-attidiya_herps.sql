...
alter table bellanwila-attidiya_herps
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists bellanwila-attidiya_herps_geom_id_btree;
create index bellanwila-attidiya_herps_geom_id_btree ON bellanwila-attidiya_herps (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists bellanwila-attidiya_herps_scientificname_btree;
create index bellanwila-attidiya_herps_scientificname_btree ON bellanwila-attidiya_herps (scientificname);
...
from bellanwila-attidiya_herps_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    bellanwila-attidiya_herps m 
LEFT JOIN bellanwila-attidiya_herps_geom g
...
update bellanwila-attidiya_herps m
set geom_id = g.cartodb_id
from bellanwila-attidiya_herps_geom g
...
SELECT * FROM get_tile('karunarathna', 'localinv', 'Peliperdix coqui','bellanwila-attidiya_herps')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'karunarathna' and type = 'localinv' or table_name = 'bellanwila-attidiya_herps'

SELECT d.*,g.*
  FROM bellanwila-attidiya_herps d
  JOIN bellanwila-attidiya_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from bellanwila-attidiya_herps where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('bellanwila-attidiya_herps');
...
select distinct scientificname from bellanwila-attidiya_herps

select distinct m.scientificname as n,
	t.common_names_eng as v
from bellanwila-attidiya_herps m left join taxonomy t 
