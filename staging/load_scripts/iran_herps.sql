...
--Replace All 
-- iran_herps with the species table name
...
alter table iran_herps
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists iran_herps_geom_id_btree;
create index iran_herps_geom_id_btree ON iran_herps (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists iran_herps_scientificname_btree;
create index iran_herps_scientificname_btree ON iran_herps (scientificname);
...
SELECT * FROM get_tile('rastegar-pouyani', 'localinv', 'Pelobates syriacus','iran_herps')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'rastegar-pouyani' and type = 'localinv' or table_name = 'iran_herps'

SELECT d.*,g.*
  FROM iran_herps d
  JOIN iran_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from iran_herps where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('iran_herps');
...
select distinct scientificname from iran_herps

select distinct m.scientificname as n,
	t.common_names_eng as v
from iran_herps m left join taxonomy t 
