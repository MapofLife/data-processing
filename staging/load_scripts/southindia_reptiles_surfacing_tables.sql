...
-- Tsetan with provider but capitalized, such as 'Stalmans'
-- Reptilian fauna of agricultural landscapes of Chembarambakkaam Lake, Chennai, Tamil Nadu with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- southindia_reptiles with dataset id (table name) e.g 'gorongosa_flora'
-- 2011 with year of publication e.g. '2010' 
-- http://www.zoosprint.org/ZoosPrintNewsLetter/Reptile_Rap_13_Sep2011.pdf#page=2 with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- Chembarambakkam Lake - the geographic extent eg. "Global" or "Angola" or "New World"
...
-- Reptile: Lacepede (1789), Daudin (1803), Schneider (1801), Shaw & Nodder (1797), Linnaeus (1758), Shaw (1802), Gray (1845), Dum. & Bib (1836), Gmelin (1799), Blyth (1853), Cuvier (1844), Lacepede (1788) with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'tsetan'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'tsetan', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'southindia_reptiles', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'southindia_reptiles', --table_name: the name of the table that holds the species list. note same as dataset_id
	'southindia_reptiles_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'tsetan', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Reptilian fauna of agricultural landscapes of Chembarambakkaam Lake, Chennai, Tamil Nadu', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'southindia_reptiles' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'southindia_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'southindia_reptiles' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'southindia_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
	'''Reptilian fauna of agricultural landscapes of Chembarambakkaam Lake, Chennai, Tamil Nadu''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'southindia_reptiles' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'southindia_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
	'''Tsetan, 2011''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'southindia_reptiles' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'southindia_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.zoosprint.org/ZoosPrintNewsLetter/Reptile_Rap_13_Sep2011.pdf#page=2''''>www.zoosprint.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'southindia_reptiles'
...
values (
	...
	'Chembarambakkam Lake', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'southindia_reptiles', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2008', --ca. 2008 with a string representation for the date range "ca. 1980 to 2010"
	'Reptilian fauna of agricultural landscapes of Chembarambakkaam Lake, Chennai, Tamil Nadu', --description - string description
	...
	'Reptile: Lacepede (1789), Daudin (1803), Schneider (1801), Shaw & Nodder (1797), Linnaeus (1758), Shaw (1802), Gray (1845), Dum. & Bib (1836), Gmelin (1799), Blyth (1853), Cuvier (1844), Lacepede (1788)', --taxonomy_metadata: references for the taxonomy
	'http://www.zoosprint.org/ZoosPrintNewsLetter/Reptile_Rap_13_Sep2011.pdf#page=2', --url - url of the datasource
