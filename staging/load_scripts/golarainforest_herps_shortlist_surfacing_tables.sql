-- wardill with provider e.g. 'stalmans'
-- Wardill with provider but capitalized, such as 'Stalmans'
-- Amphibians Recorded in Gola North - February 2009 with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- golarainforest_herps_shortlist with dataset id (table name) e.g 'gorongosa_flora'
--  with year of publication e.g. '2010' 
-- http://www.golarainforest.org/download/uploads/do_download.php?file=Herpetology+2009b.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- Gola Rainforest - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2009 with a string representation for the date range "ca. 1980 to 2010"
-- 
--  - null if none
--  - null if none
...
delete from providers where provider = 'wardill'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'wardill', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'golarainforest_herps_shortlist', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'golarainforest_herps_shortlist', --table_name: the name of the table that holds the species list. note same as dataset_id
	'golarainforest_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'wardill', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Amphibians Recorded in Gola North - February 2009', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'golarainforest_herps_shortlist' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_herps_shortlist', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'golarainforest_herps_shortlist' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_herps_shortlist', --data_table - the name of the species list table. should match data_registry.table_name
	'''Amphibians Recorded in Gola North - February 2009''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'golarainforest_herps_shortlist' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_herps_shortlist', --data_table - the name of the species list table. should match data_registry.table_name
	'''Wardill, ''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'golarainforest_herps_shortlist' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_herps_shortlist', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.golarainforest.org/download/uploads/do_download.php?file=Herpetology+2009b.pdf''''>www.golarainforest.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'golarainforest_herps_shortlist'
...
values (
	...
	'golarainforest_herps_shortlist', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2009', --ca. 2009 with a string representation for the date range "ca. 1980 to 2010"
	'Amphibians Recorded in Gola North - February 2009', --description - string description
	...
	'http://www.golarainforest.org/download/uploads/do_download.php?file=Herpetology+2009b.pdf', --url - url of the datasource
