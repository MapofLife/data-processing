-- terborgh with provider e.g. 'stalmans'
-- Terborgh with provider but capitalized, such as 'Stalmans'
-- Annotated checklist of bird and mammal species of Cocha Cashu Biological Station, Manu National Park, Peru with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- manu_mammals with dataset id (table name) e.g 'gorongosa_flora'
-- 1984 with year of publication e.g. '2010' 
-- http://archive.fieldmuseum.org/manu/pdf/cocha_cashu.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- Manu National Park - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 1973 to 1984 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- WDPA 2010 - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Mammals: Grimwood (1969), Jones and Carter (1976) with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'terborgh'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'terborgh', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'manu_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'manu_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'manu_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'terborgh', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Annotated checklist of bird and mammal species of Cocha Cashu Biological Station, Manu National Park, Peru', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'manu_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'manu_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'manu_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'manu_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Annotated checklist of bird and mammal species of Cocha Cashu Biological Station, Manu National Park, Peru''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'manu_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'manu_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Terborgh, 1984''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'manu_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'manu_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://archive.fieldmuseum.org/manu/pdf/cocha_cashu.pdf''''>archive.fieldmuseum.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'manu_mammals'
...
values (
	...
	'Manu National Park', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'manu_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 1973 to 1984', --ca. 1973 to 1984 with a string representation for the date range "ca. 1980 to 2010"
	'Annotated checklist of bird and mammal species of Cocha Cashu Biological Station, Manu National Park, Peru', --description - string description
	...
	'Mammals: Grimwood (1969), Jones and Carter (1976)', --taxonomy_metadata: references for the taxonomy
	'http://archive.fieldmuseum.org/manu/pdf/cocha_cashu.pdf', --url - url of the datasource
