...
alter table southjordan_herps
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists southjordan_herps_geom_id_btree;
create index southjordan_herps_geom_id_btree ON southjordan_herps (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists southjordan_herps_scientificname_btree;
create index southjordan_herps_scientificname_btree ON southjordan_herps (scientificname);
...
from southjordan_herps_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    southjordan_herps m 
LEFT JOIN southjordan_herps_geom g
...
update southjordan_herps m
set geom_id = g.cartodb_id
from southjordan_herps_geom g
...
SELECT * FROM get_tile('AlQuran', 'localinv', 'Peliperdix coqui','southjordan_herps')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'AlQuran' and type = 'localinv' or table_name = 'southjordan_herps'

SELECT d.*,g.*
  FROM southjordan_herps d
  JOIN southjordan_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from southjordan_herps where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('southjordan_herps');
...
select distinct scientificname from southjordan_herps

select distinct m.scientificname as n,
	t.common_names_eng as v
from southjordan_herps m left join taxonomy t 
