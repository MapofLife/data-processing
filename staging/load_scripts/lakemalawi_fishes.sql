...
--Replace All 
-- lakemalawi_fishes with the species table name
...
alter table lakemalawi_fishes
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists lakemalawi_fishes_geom_id_btree;
create index lakemalawi_fishes_geom_id_btree ON lakemalawi_fishes (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists lakemalawi_fishes_scientificname_btree;
create index lakemalawi_fishes_scientificname_btree ON lakemalawi_fishes (scientificname);
...
select the_geom
from lakemalawi_fishes_shapefile
...
SELECT  m.dist
FROM    lakemalawi_fishes m 
LEFT JOIN lakemalawi_fishes_geom g
...
update lakemalawi_fishes m
set geom_id = g.cartodb_id
from lakemalawi_fishes_geom g
...
SELECT * FROM get_tile('oliver', 'localinv', 'Serranochromis robustus','lakemalawi_fishes')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'oliver' and type = 'localinv' or table_name = 'lakemalawi_fishes'

SELECT d.*,g.*
  FROM lakemalawi_fishes d
  JOIN lakemalawi_fishes_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from lakemalawi_fishes where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('lakemalawi_fishes');
...
select distinct scientificname from lakemalawi_fishes

select distinct m.scientificname as n,
	t.common_names_eng as v
from lakemalawi_fishes m left join taxonomy t 
