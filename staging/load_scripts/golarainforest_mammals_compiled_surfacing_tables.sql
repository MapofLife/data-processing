...
-- Anadu with provider but capitalized, such as 'Stalmans'
-- A Preliminary Survey of Small Terrestrial Mammals in the Gola Forest, Sierra Leone with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- golarainforest_mammals_compiled with dataset id (table name) e.g 'gorongosa_flora'
-- 2008 with year of publication e.g. '2010' 
-- http://www.golarainforest.org/download/uploads/do_download.php?file=Small+Mammals+2008.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.golarainforest.org with the short url to the study e.g. oo.adu.org.za
...
-- Gola Forest, Sierra Leone - the geographic extent eg. "Global" or "Angola" or "New World"
...
delete from providers where provider = 'anadu'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'anadu', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'golarainforest_mammals_compiled', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'golarainforest_mammals_compiled', --table_name: the name of the table that holds the species list. note same as dataset_id
	'golarainforest_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'anadu', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'A Preliminary Survey of Small Terrestrial Mammals in the Gola Forest, Sierra Leone', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'golarainforest_mammals_compiled' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'golarainforest_mammals_compiled' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''A Preliminary Survey of Small Terrestrial Mammals in the Gola Forest, Sierra Leone (Compiled From Secondary Sources)''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'golarainforest_mammals_compiled' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''Anadu, 2008''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'golarainforest_mammals_compiled' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'golarainforest_mammals_compiled', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.golarainforest.org/download/uploads/do_download.php?file=Small+Mammals+2008.pdf''''>www.golarainforest.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'golarainforest_mammals_compiled'
...
values (
	...
	'Gola Forest, Sierra Leone', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'golarainforest_mammals_compiled', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2008', --ca. 2008 with a string representation for the date range "ca. 1980 to 2010"
	'A Preliminary Survey of Small Terrestrial Mammals in the Gola Forest, Sierra Leone (Compiled From Secondary Sources)', --description - string description
	...
	'http://www.golarainforest.org/download/uploads/do_download.php?file=Small+Mammals+2008.pdf', --url - url of the datasource
