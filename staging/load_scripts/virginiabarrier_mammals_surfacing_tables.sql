...
-- Dueser with provider but capitalized, such as 'Stalmans'
-- Mammals on the Virginia Barrier Islands with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- virginiabarrier_mammals with dataset id (table name) e.g 'gorongosa_flora'
-- 1979 with year of publication e.g. '2010' 
-- http://www.jstor.org/stable/1379822 with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.jstor.org with the short url to the study e.g. oo.adu.org.za
...
-- Virginia Barrier Islands - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 1975 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
-- Polygon created based on the map in the paper - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Mammals: Goldman (1918), Clapp et al. (1976), Bailey (1946), Mearns (1911), Handley and Patton (1947) with references for the taxonomy
...
select
	'dueser', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
where not exists (select * from providers where provider = 'dueser')
...
values (
	'virginiabarrier_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'virginiabarrier_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'virginiabarrier_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'dueser', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Mammals on the Virginia Barrier Islands', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'virginiabarrier_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'virginiabarrier_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'virginiabarrier_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'virginiabarrier_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Mammals on the Virginia Barrier Islands''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'virginiabarrier_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'virginiabarrier_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Dueser, 1979''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'virginiabarrier_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'virginiabarrier_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.jstor.org/stable/1379822''''>www.jstor.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'virginiabarrier_mammals'
...
values (
	...
	'Virginia Barrier Islands', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'virginiabarrier_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 1975', --ca. 1975 with a string representation for the date range "ca. 1980 to 2010"
	'Mammals on the Virginia Barrier Islands', --description - string description
	null, --recommended_citation 
	'none', -- none - null if none
	null, --  - null if none
	'Polygon created based on the map in the paper', -- Polygon created based on the map in the paper - what was used for spatial information
	'Mammals', -- Mammals - same as class but with common names "Plants", or "Birds"
	'Mammals: Goldman (1918), Clapp et al. (1976), Bailey (1946), Mearns (1911), Handley and Patton (1947)', --taxonomy_metadata: references for the taxonomy
	'http://www.jstor.org/stable/1379822', --url - url of the datasource
