...
-- Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- kukulugala_reptiles with dataset id (table name) e.g 'gorongosa_flora'
-- 2010 with year of publication e.g. '2010' 
-- http://www.sljol.info/index.php/TAPRO/article/view/3146/2525 with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
...
-- Kukulugala Forest - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2001 to 2002 with a string representation for the date range "ca. 1980 to 2010"
-- Karunarathna D.M.S.S, Amarasinghe A.A.T 2010. Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka. Taprobanica 2:86-94
-- none - null if none
--  - null if none
-- WDPA 2010 - what was used for spatial information
-- Reptiles - same as class but with common names "Plants", or "Birds"
-- Deraniyagala (1953, 1955), Das & De Silva (2005), De Silva (1980), De Silva (1990), Greer (1991), and Wickramasinghe & Somaweera (2003) with references for the taxonomy
...
values (
	'kukulugala_reptiles', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'kukulugala_reptiles', --table_name: the name of the table that holds the species list. note same as dataset_id
	'kukulugala_reptiles_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	...
	'Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'kukulugala_reptiles' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'kukulugala_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'kukulugala_reptiles' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'kukulugala_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
	'''Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'kukulugala_reptiles' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- delete from feature_metadata where data_table = 'kukulugala_reptiles' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'kukulugala_reptiles', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.sljol.info/index.php/TAPRO/article/view/3146/2525''''>www.sljol.info</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'kukulugala_reptiles'
...
values (
	...
	'Kukulugala Forest', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'kukulugala_reptiles', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2001 to 2002', --ca. 2001 to 2002 with a string representation for the date range "ca. 1980 to 2010"
	'Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka', --description - string description
	'Karunarathna D.M.S.S, Amarasinghe A.A.T 2010. Reptile Diversity of a Fragmented Lowland Rain Forest Patch in Kukulugala, Ratnapura District, Sri Lanka. Taprobanica 2:86-94', --recommended_citation 
	...
	'Deraniyagala (1953, 1955), Das & De Silva (2005), De Silva (1980), De Silva (1990), Greer (1991), and Wickramasinghe & Somaweera (2003)', --taxonomy_metadata: references for the taxonomy
	'http://www.sljol.info/index.php/TAPRO/article/view/3146/2525', --url - url of the datasource
