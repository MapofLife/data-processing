...
-- Lim with provider but capitalized, such as 'Stalmans'
...
-- guiana_mammals with dataset id (table name) e.g 'gorongosa_flora'
...
-- ca. 1976 to 2001 with a string representation for the date range "ca. 1980 to 2010"
...
-- Bats: Lim and Engstrom (2001), Simmons and Voss (1998). Nonvalent mammals: Voss et al. (2001), Handley (1076), Ochoa et al. (1993), Linares (1998), Linares and Rivas (2004), Lew (2001) with references for the taxonomy

select * from providers where provider = 'Lim'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'Lim', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'guiana_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'guiana_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'guiana_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'Lim', --provider: the key to the provider table (column providers.provider) eg "hillers"	
...
--delete from feature_metadata where data_table = 'guiana_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'guiana_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'guiana_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- delete from feature_metadata where data_table = 'guiana_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'guiana_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Lim, 2005''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'guiana_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'guiana_mammals'
...
values (
	...
	'guiana_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 1976 to 2001', --ca. 1976 to 2001 with a string representation for the date range "ca. 1980 to 2010"
	...
	'Bats: Lim and Engstrom (2001), Simmons and Voss (1998). Nonvalent mammals: Voss et al. (2001), Handley (1076), Ochoa et al. (1993), Linares (1998), Linares and Rivas (2004), Lew (2001)', --taxonomy_metadata: references for the taxonomy
