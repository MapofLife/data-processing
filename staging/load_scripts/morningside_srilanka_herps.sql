...
--Replace All 
-- morningside_srilanka_herps with the species table name
...
alter table morningside_srilanka_herps
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists morningside_srilanka_herps_geom_id_btree;
create index morningside_srilanka_herps_geom_id_btree ON morningside_srilanka_herps (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists morningside_srilanka_herps_scientificname_btree;
create index morningside_srilanka_herps_scientificname_btree ON morningside_srilanka_herps (scientificname);
...
from morningside_srilanka_herps_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    morningside_srilanka_herps m 
LEFT JOIN morningside_srilanka_herps_geom g
...
update morningside_srilanka_herps m
set geom_id = g.cartodb_id
from morningside_srilanka_herps_geom g
...
SELECT * FROM get_tile('janzen', 'localinv', 'Peliperdix coqui','morningside_srilanka_herps')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'janzen' and type = 'localinv' or table_name = 'morningside_srilanka_herps'

SELECT d.*,g.*
  FROM morningside_srilanka_herps d
  JOIN morningside_srilanka_herps_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Azolla nilotica';

select * from morningside_srilanka_herps where scientificname = 'Azolla nilotica';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('morningside_srilanka_herps');
...
select distinct scientificname from morningside_srilanka_herps

select distinct m.scientificname as n,
	t.common_names_eng as v
from morningside_srilanka_herps m left join taxonomy t 
