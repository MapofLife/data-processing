-- marinis with provider e.g. 'stalmans'
-- Marinis with provider but capitalized, such as 'Stalmans'
-- Mammalian fauna of the Termessos National Park, Turkey with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- termessos_mammals with dataset id (table name) e.g 'gorongosa_flora'
-- 2009 with year of publication e.g. '2010' 
-- http://www.pensoft.net/J_FILES/1/articles/171/171-G-1-layout.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.pensoft.net with the short url to the study e.g. oo.adu.org.za
...
--  - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 1994 to 2007 with a string representation for the date range "ca. 1980 to 2010"
-- De Marinis AM, Masseti M (2009) Mammalian fauna of the Termessos National Park, Turkey 
-- none - null if none
--  - null if none
-- WDPA 2010 - what was used for spatial information
-- Mammals - same as class but with common names "Plants", or "Birds"
-- Mammals: Krystufek and Vohralik 2001, Ugurluai 2005, Yortonese 1958, Kumerloeve 1967, Turan 1984, Albayrak and Erdogan 2005, Thol-Schmitz 2004, Ustay 1990, Kasparek et al. 2004 with references for the taxonomy

-- make sure there is not already a provider record for this provider
-- if there is, tell Ben.
select * from providers where provider = 'marinis'

insert into providers 
	(provider,pubdate,title,url,source_type)
values (
	'marinis', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
values (
	'termessos_mammals', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'termessos_mammals', --table_name: the name of the table that holds the species list. note same as dataset_id
	'termessos_mammals_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'marinis', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Mammalian fauna of the Termessos National Park, Turkey', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'termessos_mammals' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'termessos_mammals', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'termessos_mammals' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'termessos_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Mammalian fauna of the Termessos National Park, Turkey''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'termessos_mammals' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'termessos_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''Marinis, 2009''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'termessos_mammals' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'termessos_mammals', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.pensoft.net/J_FILES/1/articles/171/171-G-1-layout.pdf''''>www.pensoft.net</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'termessos_mammals'
...
values (
	...
	null, --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'termessos_mammals', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 1994 to 2007', --ca. 1994 to 2007 with a string representation for the date range "ca. 1980 to 2010"
	'Mammalian fauna of the Termessos National Park, Turkey', --description - string description
	'De Marinis AM, Masseti M (2009) Mammalian fauna of the Termessos National Park, Turkey ', --recommended_citation 
	...
	'Mammals: Krystufek and Vohralik 2001, Ugurluai 2005, Yortonese 1958, Kumerloeve 1967, Turan 1984, Albayrak and Erdogan 2005, Thol-Schmitz 2004, Ustay 1990, Kasparek et al. 2004', --taxonomy_metadata: references for the taxonomy
	'http://www.pensoft.net/J_FILES/1/articles/171/171-G-1-layout.pdf', --url - url of the datasource
