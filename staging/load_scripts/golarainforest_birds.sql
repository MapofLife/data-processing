...
--Replace All 
-- golarainforest_birds with the species table name
...
alter table golarainforest_birds
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists golarainforest_birds_geom_id_btree;
create index golarainforest_birds_geom_id_btree ON golarainforest_birds (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists golarainforest_birds_scientificname_btree;
create index golarainforest_birds_scientificname_btree ON golarainforest_birds (scientificname);
...
from golarainforest_birds_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    golarainforest_birds m 
LEFT JOIN golarainforest_birds_geom g
...
update golarainforest_birds m
set geom_id = g.cartodb_id
from golarainforest_birds_geom g
...
SELECT * from data_registry WHERE provider = 'dowsett_lemaire' and type = 'localinv' or table_name = 'golarainforest_birds'

SELECT d.*,g.*
  FROM golarainforest_birds d
  JOIN golarainforest_birds_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Ardea cinerea';

select * from golarainforest_birds where scientificname = 'Ardea cinerea';	
...
from golarainforest_birds m left join taxonomy t 
