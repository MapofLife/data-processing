...
--Replace All
-- golarainforest_baseline_birds with the species table name
...
alter table golarainforest_baseline_birds
add column geom_id integer;

-- create an index on the geom id <tablename>_<colname>_btree
drop index if exists golarainforest_baseline_birds_geom_id_btree;
create index golarainforest_baseline_birds_geom_id_btree ON golarainforest_baseline_birds (geom_id);

--create an index on the scientific name  <tablename>_scientificname_btree
drop index if exists golarainforest_baseline_birds_scientificname_btree;
create index golarainforest_baseline_birds_scientificname_btree ON golarainforest_baseline_birds (scientificname);
...
from golarainforest_baseline_birds_shapefile
group by geom_name;
...
SELECT  m.dist
FROM    golarainforest_baseline_birds m 
LEFT JOIN golarainforest_baseline_birds_geom g
...
update golarainforest_baseline_birds m
set geom_id = g.cartodb_id
from golarainforest_baseline_birds_geom g
...
SELECT * FROM get_tile('dowsett_lemaire', 'localinv', 'Ardea cinerea','golarainforest_birds')

-- sql to if get_tile does not work.
SELECT * from data_registry WHERE provider = 'klop' and type = 'localinv' or table_name = 'golarainforest_baseline_birds'

SELECT d.*,g.*
  FROM golarainforest_baseline_birds d
  JOIN golarainforest_baseline_birds_geom g ON 
  d.geom_id = g.cartodb_id
  where d.scientificname = 'Phalacrocorax africanus';

select * from golarainforest_baseline_birds where scientificname = 'Phalacrocorax africanus';	
...
insert into layer_metadata_staging
	select * from get_mol_layers('golarainforest_birds');
...
select distinct scientificname from golarainforest_birds

select distinct m.scientificname as n,
	t.common_names_eng as v
from golarainforest_baseline_birds m left join taxonomy t 
on m.scientificname = t.scientificname

-- talk to J - what if already in autocomplete table?
-- insert the rows
