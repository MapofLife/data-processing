...
-- Rakotondravony with provider but capitalized, such as 'Stalmans'
-- Rapid herpetofaunal surveys within five isolated forests on sedimentary rock in Western Madagascar with title e.g. 'Tinleys plant species list for the greater Gorongosa ecosystem'
-- madagascar_herps with dataset id (table name) e.g 'gorongosa_flora'
-- 2011 with year of publication e.g. '2010' 
-- http://www.herpconbio.org/Volume_6/Issue_2/Rakotondravony_Goodman_2011.pdf with the full url to the study e.g. http://oo.adu.org.za/content.php?id=4
-- www.herpconbio.org with the short url to the study e.g. oo.adu.org.za
-- herps with the lower-case tax. e.g. herps or aves
-- Herps with the same as taxa column, but capitalize
-- hey.rado@yahoo.fr with the email address of the author or organization
-- estern Madagascar - the geographic extent eg. "Global" or "Angola" or "New World"
--  with null if not applicable
-- ca. 2006 to 2007 with a string representation for the date range "ca. 1980 to 2010"
-- 
-- none - null if none
--  - null if none
...
select
	'rakotondravony', -- provider: all lower case. the id of the provider (if an article, use the author but not the date).  in the data_registry, this this is stored in the provider field.
...
where not exists (select * from providers where provider = 'rakotondravony')
...
values (
	'madagascar_herps', --dataset_id: the name of the species list table (same as table_name) e.g. "myanmar_flora"
	'madagascar_herps', --table_name: the name of the table that holds the species list. note same as dataset_id
	'madagascar_herps_geom', --geom_table: the name of the geom table.  should be <table_name>_geom eg "myanmar_flora_geom"	
	'rakotondravony', --provider: the key to the provider table (column providers.provider) eg "hillers"	
	...
	'Rapid herpetofaunal surveys within five isolated forests on sedimentary rock in Western Madagascar', --dataset_title - the publication name.  "Birds of Melanesia" should match providers.title and dashboard dataset.title
...
--delete from feature_metadata where data_table = 'madagascar_herps' and title = 'Species name'

--Species Name field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'madagascar_herps', --data_table - the name of the species list table. should match data_registry.table_name
...
-- delete from feature_metadata where data_table = 'madagascar_herps' and title = 'Source'
-- Source field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'madagascar_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Rapid herpetofaunal surveys within five isolated forests on sedimentary rock in Western Madagascar''', --field - the name of the field to be displayed from the data_table
		
	-- do not change fields below this line
	'Source', -- title - the display name of the field
	2 --order - how to order the fields from the data_table
)
-- delete from feature_metadata where data_table = 'madagascar_herps' and title = 'Provider'
-- Provider field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'madagascar_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''Rakotondravony, 2011''', --field - the name of the field to be displayed from the data_table
...
-- delete from feature_metadata where data_table = 'madagascar_herps' and title = 'URL'
-- URL field
insert into feature_metadata
	(data_table,field,title,"order") --order and type are keywords so need to be in quotes
values (
	'madagascar_herps', --data_table - the name of the species list table. should match data_registry.table_name
	'''<a target=''''_blank'''' onclick=''''window.open(this.href)'''' href=''''http://www.herpconbio.org/Volume_6/Issue_2/Rakotondravony_Goodman_2011.pdf''''>www.herpconbio.org</a>''', --field - the name of the field to be displayed from the data_table
...
-- dashboard_metadata
--delete from dashboard_metadata where dataset_id = 'madagascar_herps'
...
values (
	...
	'rakotondravony', --provider
	'hey.rado@yahoo.fr', --contact - email address of contact
	'Western Madagascar', --coverage - the geographic extent eg. "Global" or "Angola" or "New World"
	'madagascar_herps', --dataset_id - the name of the species list table
	null, --  with null if not applicable
	'ca. 2006 to 2007', --ca. 2006 to 2007 with a string representation for the date range "ca. 1980 to 2010"
	'Rapid herpetofaunal surveys within five isolated forests on sedimentary rock in Western Madagascar', --description - string description
	...
	'http://www.herpconbio.org/Volume_6/Issue_2/Rakotondravony_Goodman_2011.pdf', --url - url of the datasource
